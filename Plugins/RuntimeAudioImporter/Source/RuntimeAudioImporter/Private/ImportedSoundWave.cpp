...
TArray<float> UImportedSoundWave::GetFloatBuffer()
{	
	
	TArray<float> OutBuffer;
	int64 Length = PCMBufferInfo.PCMDataSize / sizeof(float);
	const float* Data = (float*)(PCMBufferInfo.PCMData);
	for (int64 i = 0; i < Length; ++i) {
		OutBuffer.Add(Data[i]);
	}
	return OutBuffer;
}